...
async def fetch_and_save(symbol, timeframe, exchange, semaphore: asyncio.Semaphore):
    async with semaphore:  # This will limit the number of concurrent tasks
        ...
        if os.path.exists(filename):
            logging.info(f"Data for {symbol} on timeframe {timeframe} already exists. Skipping download.")
            return
        ...
        while True:
            try:
                ...
                if retry_count > max_retries:
                    logging.error(f"Max retries exceeded for {symbol} on timeframe {timeframe}.")
                    return
                logging.warning(f"Timeout error for {symbol} on timeframe {timeframe}. Retrying... Attempt {retry_count}")
                await asyncio.sleep(2 ** retry_count)  # Exponential backoff

        if all_ohlcv:
            df = pd.DataFrame(all_ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
            df.to_csv(filename, index=False)
            logging.info(f"Completed fetching {len(all_ohlcv)} entries for {symbol} on timeframe {timeframe}. Data saved to {filename}")
...
async def main():
    ...
    tasks = [fetch_and_save(symbol, timeframe, exchange, semaphore) for symbol in symbols for timeframe in timeframes]
