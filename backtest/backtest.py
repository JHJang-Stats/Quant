...
class Backtest:
    def __init__(self, data, strategy, start_date=None, end_date=None, fee=2e-4, leverage=1, enable_logging=False):
        self.data = data
    ...
    def simulate_trades(self, initial_capital=10000):
        ...
        self.portfolio.at[self.portfolio.index[0], 'change_signal'] = self.portfolio['signal'].iloc[0]
        ...
        for idx, row in self.portfolio.iterrows():
            ...
            if prev_signal == 0:
                if row['change_signal'] != 0:
                    if row['signal']==0:
                        ...
                        prev_contract_numbers = prev_cash / prev_entry_price * self.leverage * prev_signal # contract number 음수일시 sell position
            ...
            # position 존재할 때
                ...
                if row['change_signal'] == 0:
                    ...
                    # 청산 진행 (청산시 수수료 고려)
                    prev_entry_price = None
                    prev_cash = prev_cash + self.portfolio.at[idx, 'close'] * prev_contract_numbers * (1 - prev_signal * self.fee)
                    prev_signal = 0
                    prev_contract_numbers = 0
            ...
            self.portfolio.at[idx, 'total'] = prev_cash + self.portfolio.at[idx, 'close'] * prev_contract_numbers
    ...
    def _log_position_changes(self):
        ...
        for date, change in changes.iterrows():
            ...
            position_type = "Long" if change['signal'] > 0 else "Short" if change['signal'] < 0 else "Flat"
            entry_price = f"{change['entry_price']:.4f}" if change['entry_price'] is not None else 'N/A'
            log_message = (
    ...
    def calculate_metrics(self, risk_free_rate=0.0):
        ...
        self.portfolio['time_diff'] = self.portfolio.index.to_series().diff().dt.total_seconds() / (60*60*24)
        self.portfolio['excess_returns'] = self.portfolio['portfolio_returns'] - risk_free_rate / (365 / self.portfolio['time_diff'])
        ...
        if excess_returns.std() == 0:
            ...
            sharpe_ratio = excess_returns.mean() / excess_returns.std() * np.sqrt(avg_intervals_per_year)
        ...
        self.portfolio.drop(columns=['portfolio_returns', 'time_diff', 'excess_returns'], inplace=True)
