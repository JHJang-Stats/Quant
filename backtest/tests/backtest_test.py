...
def baseline_metrics():
    ...
    backtest = Backtest(market_data.data, strategy, start_date='2018-01-01', end_date=None, fee=2e-4, enable_logging=True)
    backtest.run()
...
def test_strategy_metrics(file_path, baseline_metrics):
    ...
    for metric in ['sharpe_ratio', 'max_drawdown']:
        assert abs(metrics[metric] - baseline_metric[metric]) / baseline_metric[metric] <= 0.02, f"{metric} not within 2% of baseline"
    
    final_total = portfolio['total'].iloc[-1]
    assert abs(final_total - baseline_total) / baseline_total <= 0.02, "Final total not within 2% of baseline"
