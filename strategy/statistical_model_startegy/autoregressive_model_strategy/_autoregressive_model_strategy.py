...
class ARModelStrategy(StatisticalModelStrategy):
    def __init__(self, data, lags=1, fit_start_date=None, fit_end_date=None, predict_steps=1, thresholds=0.004):
        super().__init__(data)
    ...
    def generate_signals(self):
        ...
        self.signals['signal'][self.predictions['Y_hat'] > self.predictions['close'] * (1 + self.thresholds)] = 1
        self.signals['signal'][self.predictions['Y_hat'] < self.predictions['close'] * (1 - self.thresholds)] = -1
