...
class ADLStrategy(IndicatorBasedStrategy):
    ...
    def generate_signals(self):
        ...
        mfm = (
            (copy_data["close"] - copy_data["low"])
            - (copy_data["high"] - copy_data["close"])
        ) / (copy_data["high"] - copy_data["low"])
        ...
        self.signals["signal"][1:] = np.where(
            copy_data["adl"][1:] > copy_data["adl"][:-1].values, 1, -1
        )
