...
class StochasticOscillatorStrategy(Strategy):
    ...
    def generate_signals(self):
        ...
        self.signals.loc[(copy_data['%K'].shift(1) < self.oversold_threshold) & (copy_data['%K'] > self.oversold_threshold), 'signal'] = 1
        self.signals.loc[(copy_data['%K'].shift(1) > self.overbought_threshold) & (copy_data['%K'] < self.overbought_threshold), 'signal'] = -1
